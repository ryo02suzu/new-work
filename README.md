...
- **CSV出力設定（基幹に合わせる）**：使う列のON/OFF・列名の変更・順序の入れ替えを設定し、御社の基幹/Excelの取り込み項目にCSVを合わせられる（端末に保存）。＝パイロットで「そのまま取り込める」を実現し、乗り換えコストにもなる。
- **複数注文の一括処理**：「---」区切りのテキスト or 画像を複数選択 → まとめてデータ化 → 全件を1つのCSVに（`/api/extract-batch`）。「今日ぶんの注文」を一気に処理する実運用形。
...
最速の現金化は**「御社の注文書1週間ぶんを無料でデータ化（コンシェルジュMVP）→数字の出た事例→同業へ横展開」**。価格・GTM・検証基準・週次アクションは [ROADMAP.md](./ROADMAP.md) に、**最初の1社を取るための営業文面・パイロット資料・デモ台本は [GTM-KIT.md](./GTM-KIT.md)** にまとめてある。
